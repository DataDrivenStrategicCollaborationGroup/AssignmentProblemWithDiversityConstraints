...
public class MainTests {
 ...
 public static ArrayList<ArrayList<Double>> planningAreaAttribs = new ArrayList<ArrayList<Double>>(); //1st two columns: longitude, latitude; next 3 rows Chinese, Malay, Indian/Others population sizes (Master Plan 2014: data.gov.sg/dataset/master-plan-2014-planning-area-boundary-web)
 public static int numPlanningAreas = -1;
 public static double[] ethnoPops = new double[]{2899866.65,521066.65,481896.65}; //Populations of Chinese, Malay, Indian/Others in Singapore from Master Plan 2014 (ProjectBasedModelData.xlsx): ethnic breakdown unavailable for planning areas with population <=100 but we assume equal ethnic populations (divide by 3); .65 instead of .67 to be consistent with values in .txt file ending in .33, so that ethnic populations of all planning areas add up to corresponding total ethnic population in Singapore
 public static ArrayList<int[]> capacities = new ArrayList<int[]>();
 public static int numTypes = -1; //For Singapore, 0=Chinese, 1=Malay, 2=Indian/Others
 public static int[] numAgentPerType = null;
 public static int numAgents = 0;
 public static ArrayList<Agent> agents = null;
 public static double variance = 10;// for location/distance-based and ethnicity-based: variance of Gaussian utilities
 public static double radius = 5;// for project-based: agent approves of any flat in any project within this radius of its preferred location 
 
 
//////////////////////////////////////////////////////////////////////

 public static void generateRandomHDBData(int numBlocks, int numAptPerBlock, int numAgents, int[] numAgentsPerType, int numTypes, boolean locationBased){//Not based on real HDB data, all locations are randomly generated in a unit square: for preliminary tests on location/distance-based and ethnicity-based
...
//////////////////////////////////////////////////////// 
 ...
 public static double lotteryProcess(){//for location- and ethnicity-based
  ...
  for (int i=0;i<MainTests.numAgents;i++){
   ...
   int agent = ordering.remove(0);//this agent must choose a flat at this step
 ...
 public static double lotteryProcessProj(){//for project-based model where we have {0,1} utilities, hence agent arbitrarily chooses flat in an approved block that is not filled up or has not hit capacity for agent's type
  
  double totalUtility = 0;
  
  //Generate one agent permutation:
  ArrayList<Integer> ordering = new ArrayList<Integer>();
  for (int i=0;i<MainTests.numAgents;i++){//Remove agents who approve of no flats (Ideally there should be no such agents)
   if (MainTests.agents.get(i).getApprovedBlocks().size() > 0){
    ordering.add(i);
   }
  }
  Collections.shuffle(ordering);
  ...
  //Create variables for current capacities and flat availabilities:
  ArrayList<int[]> currentCapacities = new ArrayList<int[]>();
  int[] aptsLeft = new int[MainTests.numBlocks];
  for (int b=0;b<MainTests.numBlocks;b++){
   aptsLeft[b] = MainTests.numAptPerBlock[b];
   int[] currentCapacitiesb = new int[MainTests.numTypes];
   for (int l=0;l<MainTests.numTypes;l++){
    currentCapacitiesb[l] = MainTests.capacities.get(b)[l];
   }
  ...
  //Simulate flat selections:
  int numAgentsToAssign = ordering.size();
  for (int i=0;i<numAgentsToAssign;i++){
   
   int agent = ordering.remove(0);//this agent must choose a flat at this step
   int type = MainTests.agents.get(agent).getType();
   
   //Blocks that this agent approves of (each flat in each such block is valued at 1):
   ArrayList<Integer> blocksApproved = MainTests.agents.get(agent).getApprovedBlocks();
   
   
   //Find best available item for the agent:
   for (int b=0;b<blocksApproved.size();b++){
    int block_b = blocksApproved.get(b);
    if (aptsLeft[block_b] > 0 && currentCapacities.get(block_b)[type] > 0){//if the block has flats left and the type capacity has not yet been reached
     //choose a random flat in this block and assign it to this agent 
     currentCapacities.get(block_b)[type] = currentCapacities.get(block_b)[type]-1;
     aptsLeft[block_b] = aptsLeft[block_b] - 1;
 ...
 public static void main(String[] args){
  ...
  boolean HDB_realData = true; // run experiments using the data collected from HDB website
  ...
  if (HDB_realData){
    ...
    int numAgentsToBeCreated = 1350;
   
    // set only one the following variables to true:
    boolean randomUtility = false; //random normalized utility model
    ...
    //Load data on planning areas
    ProcessResultsFile filePlanningArea = new ProcessResultsFile();
    filePlanningArea.runParser("Data/housing/planningArea");
    MainTests.planningAreaAttribs = filePlanningArea.getVectors();
    ...
    MainTests.numAgentPerType = new int[numTypes];
    MainTests.numPlanningAreas = MainTests.planningAreaAttribs.size();//project-based
    int tmp = 0;
    for (int b=0;b<MainTests.numBlocks;b++){
    ...
    for (int l=0;l<MainTests.numTypes;l++){
     MainTests.numAgentPerType[l] = (int)(numAgentsToBeCreated*actual_prop[l]);
    ...
    //Compute min and max coordinates for agents' preferred locations: for distance- and ethnicity-based only
    ...
    double maxDistInMap = Math.sqrt(Math.pow(minCoordinate[0]-maxCoordinate[0], 2) + Math.pow(minCoordinate[1]-maxCoordinate[1], 2));
    ...
    for (int t=0;t<numTests;t++){
     ...
     if (randomUtility){//preliminary: not in the paper
      ...
      try {
       ...
       ArrayList<ArrayList<Double>> opts = fileOctave.getVectors();
       double average = 0;
     ...
     if (locationBasedUtility){// distance-based on HDB
     ...
     if (locationEthnicityBasedUtility){//ethnicity/type-based HDB
      int[] vars ={1,5,10};
     ...
     if (projectBasedUtility){//project-based HDB
  
      for (int c=0;c<3;c++){
      
       MainTests.radius = 5+2.5*c;
       
       //Generate result files:
       ProcessResultsFile fileResults = new ProcessResultsFile();
       fileResults.createResultsFile(directory+"projectBased-radius"+MainTests.radius);
       String fileError=directory+"error-projectBased-radius"+MainTests.radius+".txt";
       
       //Generate the agents (with project-based utilities):
       MainTests.agents = new ArrayList<Agent>();     
       ArrayList<Double> results = new ArrayList<Double>();
       double averagedApprovedBlocks = 0;
       //double[] thisPlanningAreaAttribs = new double[5];
       for (int l=0;l<MainTests.numTypes;l++){
        for (int i=0;i<MainTests.numAgentPerType[l];i++){
         double[] bestPosition = new double[2]; 
         double randNum = MainTests.ethnoPops[l]*Math.random();
         double cumProb = 0.0;
         for (int j=0;j<MainTests.numPlanningAreas;j++){//sample a planning area (mid-point) with probability proportional to population of this ethnicity in that area
          cumProb = cumProb + MainTests.planningAreaAttribs.get(j).get(2+l);
          if (randNum <= cumProb) {
           bestPosition[0] = MainTests.planningAreaAttribs.get(j).get(0);
           bestPosition[1] = MainTests.planningAreaAttribs.get(j).get(1);
           break;
          }
         }
         double maxDistFromLoc = MainTests.radius;
         Agent a = new Agent(l,maxDistFromLoc,bestPosition); 
         averagedApprovedBlocks = averagedApprovedBlocks+ a.getApprovedBlocks().size();
         MainTests.agents.add(a);
      
        } 
       }
       averagedApprovedBlocks = averagedApprovedBlocks*1.0/(MainTests.numBlocks*MainTests.numAgents);
       System.out.println("Percentage of approved blocks on average = "+averagedApprovedBlocks);
       System.out.println("NumAgents = "+MainTests.agents.size());
   
       //Compute OPT:
       double optWithout = Solver.assignmentWithOrWithoutConstraintsHDB(false);
       results.add(optWithout);
       System.out.println("end opt = "+optWithout);
    
       //Compute OPT_C:
       double optConstraints = Solver.assignmentWithOrWithoutConstraintsHDB(true);
       results.add(optWithout/optConstraints);
       System.out.println("end optConstraints = "+(optWithout/optConstraints));
    
       //Compute Lot:
       double meanLot=0;
       int numLot = 50;
       for (int l=0;l<numLot;l++){
        meanLot = meanLot+ MainTests.lotteryProcessProj();
       }
       meanLot = meanLot*1.0/numLot;
       results.add(optWithout/meanLot);
       System.out.println("end lottery = "+(optWithout/meanLot));
       ...
       //Percentage of approved blocks on average (per agent):
       results.add(averagedApprovedBlocks);
    
       //Write results:
       fileResults.addVector(results, results.size(), 0);
       ProcessResultsFile fileOctave = new ProcessResultsFile();
       fileOctave.runParser(directory+"projectBased-radius"+MainTests.radius);
       fileOctave.writeOctaveMeans("totalUtilitiesBound1BetaBound2PercentageApprovedBlocks");
     ...
     if (incomePriceBasedUtility){//price-based HDB
   ...
   if (chicago){//Chicago public school choice
